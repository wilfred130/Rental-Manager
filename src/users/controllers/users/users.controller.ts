import { Body, Controller, Delete, Get, Inject, Param, ParseIntPipe, Patch, Post, Put, UseGuards, UsePipes, ValidationPipe } from '@nestjs/common';
...
export class UsersController {
    ...
    createUser(@Body(new ValidationPipe(), ValidateCreateUserPipe) createUserDetails: CreateUserDto) {
        return this.usersService.create(createUserDetails);
    }

    @UseGuards(JwtAuthGuard)
    @Put(':id')
    updateUser(@Param('id', ParseIntPipe, IdExistsPipe) id: number, @Body(new ValidationPipe(), ValidateUpdateUserPipe, EmailExistsPipe, UsernameExistsPipe) updateUserDto: UpdateUserDto) {
